...
BEGIN
  ...
  IF _email_domain IS NOT NULL AND length(_email_domain) > 0 THEN
    ...
    WHERE _email_domain = ANY (SELECT lower(d) FROM unnest(allowed_email_domains) AS d)
    ORDER BY created_at ASC LIMIT 1;
    IF FOUND THEN
      INSERT INTO public.profiles (id, company_id, display_name, approval_status, pending_role)
      VALUES (NEW.id, _matched_company.id,
        COALESCE(NEW.raw_user_meta_data->>'display_name', split_part(NEW.email,'@',1)),
  ...
  INSERT INTO public.companies (name, plan) VALUES (_company_name, 'trial') RETURNING id INTO _company_id;
...
AS $function$
DECLARE _inv public.invitations%ROWTYPE;
BEGIN
...
BEGIN
  ...
  UPDATE public.profiles
   ...
   WHERE id = _user_id;
  INSERT INTO public.user_roles (user_id, role, company_id)
  VALUES (_user_id, _role, _cid) ON CONFLICT DO NOTHING;
  SELECT email INTO _user_email FROM auth.users WHERE id = _user_id;
  IF _user_email IS NOT NULL THEN
    UPDATE public.invitations SET accepted_at = now()
     WHERE company_id = _cid AND lower(email) = lower(_user_email) AND accepted_at IS NULL;
  END IF;
  INSERT INTO public.audit_logs (company_id, table_name, record_id, action, actor_id, after)
  VALUES (_cid, 'profiles', _user_id, 'approve', auth.uid(), jsonb_build_object('role', _role));
  INSERT INTO public.notifications (user_id, company_id, kind, title, body, severity, link)
  VALUES (_user_id, _cid, 'approval_granted', 'Your account has been approved',
          'You now have access to the workspace.', 'success', '/app');
END $function$;
...
BEGIN
  ...
  IF NOT public.has_role(auth.uid(), 'super_admin'::public.app_role) THEN
    RAISE EXCEPTION 'Super admin role required'; END IF;
  IF _user_id = auth.uid() THEN RAISE EXCEPTION 'Cannot reject yourself'; END IF;
  UPDATE public.profiles
  ...
  INSERT INTO public.audit_logs (company_id, table_name, record_id, action, actor_id, after)
  VALUES (_cid, 'profiles', _user_id, 'reject', auth.uid(), jsonb_build_object('reason', _reason));
END $function$;
...
CREATE OR REPLACE FUNCTION public.notify_super_admins_on_pending()
 RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE _admin uuid; _name text;
BEGIN
  IF NEW.approval_status <> 'pending' OR NEW.company_id IS NULL THEN RETURN NEW; END IF;
  _name := COALESCE(NEW.display_name, 'A new user');
  FOR _admin IN SELECT user_id FROM public.user_roles
...
AFTER INSERT ON public.profiles FOR EACH ROW EXECUTE FUNCTION public.notify_super_admins_on_pending();
...
AFTER UPDATE ON public.profiles FOR EACH ROW EXECUTE FUNCTION public.notify_super_admins_on_pending_update();