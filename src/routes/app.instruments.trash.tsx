...
export const Route = createFileRoute("/app/instruments/trash")({
  component: TrashPage,
});
...
function TrashPage() {
  ...
  const load = async () => {
    ...
    setRows(null);
    const { data, error } = await (supabase.from("instruments" as any) as any)
      .select("id, asset_id, name, category, serial_number, status, deleted_at")
  ...
  if (!isAdmin) {
    return (
      <ModulePage title="Instruments Trash" subtitle="Restricted to super admins.">
        <div className="text-center py-20 text-muted-foreground">You don't have permission to view the instruments trash.</div>
      </ModulePage>
    );
  }
  ...
  const restore = async (id: string) => {
    setBusy(id);
    const { error } = await (supabase.rpc as any)("restore_instrument", { _id: id });
    setBusy(null);
  ...
  const hardDelete = async (id: string) => {
    if (!(await confirmDialog("Permanently delete this instrument? This cannot be undone."))) return;
    setBusy(id);
    const { error } = await (supabase.from("instruments" as any) as any).delete().eq("id", id);
    setBusy(null);
    if (error) return toast.error(error.message);
  ...
  return (
    <ModulePage
      title="Instruments Trash"
      subtitle="Soft-deleted QA/QC instruments. Restore to bring them back into active records."
      action={
        <Link to="/app/instruments">
          <Button size="sm" variant="outline"><ArrowLeft className="size-4 me-1" /> Back to Instruments</Button>
        </Link>
      }
    >
      <div className="overflow-x-auto rounded-md border border-border">
        <table className="w-full text-sm">
          <thead className="text-xs text-muted-foreground bg-muted/40">
            <tr>
              <Th>Asset ID</Th><Th>Name</Th><Th>Category</Th><Th>Serial</Th><Th>Status</Th><Th>Deleted</Th>
            ...
            {rows?.map((r) => (
              <tr key={r.id} className="border-t border-border/60 hover:bg-muted/20">
                <td className="px-5 py-3 font-mono text-xs">
                  <button onClick={() => nav({ to: "/app/instruments/$instrumentId", params: { instrumentId: r.id } })} className="hover:text-primary">{r.asset_id}</button>
                </td>
