...
export const Route = createFileRoute("/app/pwps/trash")({
  component: TrashPage,
});
...
function TrashPage() {
  ...
  const load = async () => {
    ...
    const { data, error } = await (supabase.from("pwps" as any) as any)
      .select("id, pwps_no, title, revision, status, code_family, process, deleted_at")
  ...
  const restore = async (id: string) => {
    setBusy(id);
    const { error } = await (supabase.rpc as any)("restore_pwps", { _id: id });
    setBusy(null);
  ...
  const hardDelete = async (id: string) => {
    ...
    setBusy(id);
    const { error } = await (supabase.from("pwps" as any) as any).delete().eq("id", id);
    setBusy(null);
    if (error) return toast.error(error.message);
            ...
            {rows?.map((r) => (
              <tr key={r.id} className="border-t border-border/60 hover:bg-muted/20">
                <td className="px-5 py-3 font-mono text-xs">
                  <button onClick={() => nav({ to: "/app/pwps/$pwpsId", params: { pwpsId: r.id } })} className="hover:text-primary">{r.pwps_no}</button>
                </td>
                <td className="px-5 py-3 text-muted-foreground">{r.title ?? "—"}</td>
