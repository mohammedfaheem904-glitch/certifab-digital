...
export const PRINT_MODES: { id: WpsPrintMode; label: string; description: string; watermark?: string; hide?: string[] }[] = [
...
export function getPrintMode(id?: string | null): typeof PRINT_MODES[number] {
  return PRINT_MODES.find((m) => m.id === id) ?? PRINT_MODES[1];
}

/** Bulk export procedures to CSV with engineering-relevant columns. */
export function bulkExportProceduresCsv(rows: any[], filename = "wps-export") {
...
export async function bulkExportProceduresXlsx(rows: any[], filename = "wps-export") {
  const flat = rows.map((p) => ({
    Code: p.code,
    "WPS No": p.wps_no ?? "",
    "PQR No": p.pqr_no ?? "",
...
/** Heuristic readiness scoring from procedure header alone (dashboard-scale). */
export type WpsHeaderReadiness = {
  score: number;
  bucket: "Ready" | "Attention" | "High Risk" | "Critical";
  reasons: string[];
};

export function scoreProcedureHeader(p: any): WpsHeaderReadiness {
  const reasons: string[] = [];
  let penalty = 0;
  if (!p.pqr_no) { reasons.push("No supporting PQR linked"); penalty += 25; }
  if (!p.position_qualified && !p.position) { reasons.push("Position not qualified"); penalty += 8; }
  if (!p.base_material) { reasons.push("Base material missing"); penalty += 12; }
  if (!p.filler_material) { reasons.push("Filler material missing"); penalty += 10; }
  if (p.voltage_min == null || p.current_min == null) { reasons.push("Electrical parameters incomplete"); penalty += 15; }
  if (p.heat_input_min == null && p.heat_input_max == null) { reasons.push("Heat input window not defined"); penalty += 6; }
  if (p.status === "Draft") { reasons.push("Still in Draft"); penalty += 4; }
  if (p.status === "Rejected") { reasons.push("Procedure rejected"); penalty += 40; }
  if (!p.approved_at && (p.status === "Approved" || p.status === "Released")) {
    reasons.push("Approval timestamp missing");
    penalty += 8;
  }
  const score = Math.max(0, 100 - penalty);
  const bucket: WpsHeaderReadiness["bucket"] =
    score >= 85 ? "Ready" : score >= 65 ? "Attention" : score >= 40 ? "High Risk" : "Critical";
  return { score, bucket, reasons };
}

export function distribution<T extends Record<string, any>>(rows: T[], key: keyof T): { label: string; count: number }[] {
  const m = new Map<string, number>();
  for (const r of rows) {
    const raw = r[key];
    const k = raw == null || raw === "" ? "—" : String(raw);
    // split combined processes like "GTAW+SMAW"
