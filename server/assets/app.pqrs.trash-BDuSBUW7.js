...
function TrashPage() {
  ...
  const load = async () => {
    ...
    const {
      data,
      error
    } = await supabase.from("pqrs").select("id, pqr_no, status, code_family, standard, overall_result, deleted_at").eq("company_id", profile.company_id).not("deleted_at", "is", null).order("deleted_at", {
      ascending: false
    });
    if (error) {
  ...
  if (!isAdmin) {
    return /* @__PURE__ */ jsxRuntimeExports.jsx(ModulePage, { title: "PQR Trash", subtitle: "Restricted to super admins.", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-center py-20 text-muted-foreground", children: "You don't have permission to view the PQR trash." }) });
  }
  const restore = async (id) => {
    setBusy(id);
    const {
      error
    } = await supabase.rpc("restore_pqr", {
      _id: id
    });
    setBusy(null);
  ...
  const hardDelete = async (id) => {
    ...
    const {
      error
    } = await supabase.from("pqrs").delete().eq("id", id);
    setBusy(null);
    if (error) return toast.error(error.message);
    toast.success("Permanently deleted.");
    load();
  };
  return /* @__PURE__ */ jsxRuntimeExports.jsx(ModulePage, { title: "PQR Trash", subtitle: "Soft-deleted Procedure Qualification Records. Restore to bring them back into active records.", action: /* @__PURE__ */ jsxRuntimeExports.jsx(Link, { to: "/app/pqrs", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(Button, { size: "sm", variant: "outline", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("tbody", { children: [
      ...
      rows?.map((r) => /* @__PURE__ */ jsxRuntimeExports.jsxs("tr", { className: "border-t border-border/60 hover:bg-muted/20", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-5 py-3 text-muted-foreground", children: r.standard ?? "—" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-5 py-3", children: r.status }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("td", { className: "px-5 py-3", children: r.overall_result }),
